...
[Functions]
    Water_Z_Ref = 0.06 #TO CHECK WITH JÖRG-> add pore pressure after confinement!
...
[BCs]
    [front_pfs]
        type = PorousFlowSink
        boundary = 'BoundaryXMax BoundaryXMin BoundaryYMax BoundaryYMin BoundaryZMax BoundaryZMin' #TO CHECK WITH JÖRG LATERAL GROUPS
        variable = 'porepressure'
        flux_function = 0.0
    []
[]
...
# apply confinement pressures with StagedFunctions
[BCs] #CHECK WITH JÖRG THE GROUPS FOR LATERAL SURFACES
    [sig_z]
        type = Pressure
        variable = disp_z
        boundary = 'BoundaryZMax'
...
[Stages]
    ...
    [Stage1] #CHECK WITH JÖRG WETHER LOADS SHOULD BE APPLIED OR USE INITIAL PORE PRESSURE AFTER CONSOLIDATION PHASE AS BOUNDARY CONDITIONS
        load_x = '${units 2500 kN/m^2 -> ${modelunit_pressure} }'
        load_y = '${units 2500 kN/m^2 -> ${modelunit_pressure} }'
        load_z = '${units 2500 kN/m^2 -> ${modelunit_pressure} }'

        #lets activate the confinement change values from 0 to 2500 kN/m^2
        t = 1.0
    ...
    [Stage3]
        ...
        t = 2.0
        constant_strain_z = '${units 5.0e-7 1/s -> ${modelunit_strain_rate} }' #CHECK WITH JÖRG WETHER UNIT FORMAT IS CORRECTLY USED
...
[Materials]
    ...
    [eigenstrain]
        ...
        initial_stress = 'ini_xx 0 0  0 ini_yy 0  0 0 ini_zz' #CHECK WITH JÖRG WETHER INITIAL STRESS COMPUTATION IS OF INTEREST
    []
