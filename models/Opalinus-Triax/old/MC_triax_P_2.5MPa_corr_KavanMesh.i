...
[Variables]
    ...
    [porepressure]
        ...
        #order = SECOND
        order = FIRST
        scaling = 1E10 #for a MC-Model should be fine -> later for a Opalinus Material a scaling factor of 1E+9 is to be used
    []
[]
...
[Functions]
    ...
    [func_confinement_pressure]
        ...
        # #01 the following gives too much confinement (the sample gets squeezed)
        # expression = '(${Water_Z_Ref} - z) * ${material_density} * ${gravitational_acceleration} + ${pconf_total}' #pconf
        
        # #02 
        # expression = '(${Water_Z_Ref} - z) * ${water_density} * ${gravitational_acceleration} + ${pconf_total}'

        # #03
        # expression = '${pconf_total}'

        # #04
        # expression = '${pconf} + 0*${pconf_total}'

        # #05
        # expression = '0*${pconf} + 0*${pconf_total}'

        # #06
        # expression = '${pconf} * 1/2 + 0*${pconf_total}'

        # #07
        # expression = '${pconf} * 3/4 + 0*${pconf_total}'

        # #08
        # expression = '${pconf} + 0*${pconf_total} - (${Water_Z_Ref} - z) * ${water_density} * ${gravitational_acceleration}'

        # #09
        #expression = '0*${pconf} + ${pconf_total} - (${Water_Z_Ref} - z) * ${water_density} * ${gravitational_acceleration}'
...
[ICs]
    [porepressure]
        type = FunctionIC
        variable = 'porepressure' #apply 2.5 MPs porepresure
...
[BCs]
    [front_pfs]
        type = PorousFlowSink
        boundary = 'bala zir baghal'
        #boundary = '${Mesh/MantleSurfaces} ${Mesh/BoundaryZMax} ${Mesh/BoundaryZMin}'
        #boundary = 'back bottom right top left front'
        variable = 'porepressure'
        flux_function = 0.0
    []
[]
